    ...
    try {
      ...
      let data = await StoreAll.findOne({
        where: {
          sid: P.sid,
          cid: P.cid,
          size: `${P.size}(${P.g_name})` // 都在这里 判断也行
        }
      })
      if (!data) {
  ...
  async comeIn(req, res) {
    ...
    try {
      ...
      if (Number(P.in_reason) === 1) {
        // 正常原因
        if (data === null) {
          var gid = shortid.generate(); // 添加 新入库的商品时应统一 gid
          console.log('!!!!!!没有数据，添加新的数据')
          var option = {
            id: P.id,
            sid: P.sid,
            s_name: P.s_name,
            ...
            gid, // 随机gid
            g_name: P.g_name,
            size: `${P.size}(${P.g_name})`, // g_name + size
            all_num: P.in_num, // 总数量
            all_unit: P.in_unit,
            all_at: P.in_at,
            per_price: P.per_price,
            ...
            person: P.person,
            mark: P.mark,
            all_reason: P.in_reason,
            op: P.op // 该商品所属供货商          
          }
          // 库存表
          await StoreAll.create(option, {
            transaction
          });
          ...
          // 入库表
          data = await StoreIn.create(option2, {
            transaction
          });
          res.send({
            code: 0,
